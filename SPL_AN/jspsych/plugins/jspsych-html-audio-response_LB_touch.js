/**
 * jspsych-html-audio-response_LB
 ...
 * Becky Gilbert, Apr 2020 (https://github.com/becky-gilbert)
  * Hannah Small, 2020/07/07 (https://github.com/hesmall)
    * Lawrence Binding, 2020/09/19 ~ displays images instead + added delay when pushing button to stop recording
 ...
 *
 **/
document.addEventListener("touchstart", touchToSpace);

function touchToSpace(event) {
    // Check if the event was triggered by a touchscreen
    if (event.pointerType === "touch") {
      // Cancel the default action of the touch event to prevent scrolling or zooming
      event.preventDefault();
  
      // Create a new KeyboardEvent with the space key and dispatch it to the target element
      var spaceEvent = new KeyboardEvent("keydown", {key: " ", code: "Space"});
      event.target.dispatchEvent(spaceEvent);
    }
  }
...
jsPsych.plugins["html-audio-response"] = (function() {
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            recording_light: {
                ...
                default: '<div id="jspsych-html-audio-response_LB-light" '+
    ...
    plugin.trial = function(display_element, trial) {
        ...
        // add stimulus
        // just text
        //let html = '<div id="jspsych-html-audio-response_LB-stimulus">'+trial.stimulus+'</div>';
        //image
        var html = '<div id="imageHolder"><img src="'+trial.stimulus+'" id="jspsych-html-audio-response_LB-stimulus" style=" </div>';
        ...
        html += '<div id="jspsych-html-audio-response_LB-recording-container">'+trial.recording_light_off+'</div>';

        // add audio element container with hidden audio element
        html += '<div id="jspsych-html-audio-response_LB-audio-container"><audio id="jspsych-html-audio-response_LB-audio" controls style="visibility:hidden;"></audio></div>';

        // add button element with hidden buttons
        html += '<div id="jspsych-html-audio-response_LB-buttons"><button id="jspsych-html-audio-response_LB-rerecord" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">Retry</button><button id="jspsych-html-audio-response_LB-okay" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">Sounds good!</button></div>';

        function start_trial() {
            display_element.innerHTML = html;
            document.querySelector('#jspsych-html-audio-response_LB-okay').addEventListener('click', end_trial);
            document.querySelector('#jspsych-html-audio-response_LB-rerecord').addEventListener('click', start_recording);
            // Add visual indicators to let people know we're recording
            document.querySelector('#jspsych-html-audio-response_LB-recording-container').innerHTML = trial.recording_light;
            // trial start time
            start_time = performance.now();
            // set timer to hide-html if stimulus duration is set
            if (trial.stimulus_duration !== null) {
                jsPsych.pluginAPI.setTimeout(function() {
                    display_element.querySelector('#jspsych-html-audio-response_LB-stimulus').style.visibility = 'hidden';
        ...
        var after_response = function(info){
            ...
            display_element.querySelector('#jspsych-html-audio-response_LB-stimulus').className += ' responded';
        ...
        function showPlaybackTools(data) {
            // Audio Player
            let playerDiv = display_element.querySelector('#jspsych-html-audio-response_LB-audio-container');
            let url;
            ...
            let player = playerDiv.querySelector('#jspsych-html-audio-response_LB-audio');
            player.src = url;
            player.style.visibility = "visible";
            // Okay/rerecord buttons
            let buttonDiv = document.querySelector('#jspsych-html-audio-response_LB-buttons');
            let rerecord = buttonDiv.querySelector('#jspsych-html-audio-response_LB-rerecord');
            let okay = buttonDiv.querySelector('#jspsych-html-audio-response_LB-okay');
        ...
        function onRecordingFinish(data) {
            ...
            let light = document.querySelector('#jspsych-html-audio-response_LB-recording-container');
            if (light !== null)
